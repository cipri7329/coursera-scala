...
class StackOverflow extends Serializable {
  ...
  */
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questionPostings = postings.filter(posting => posting.postingType == questionType).map(posting => (posting.id, posting))
    val answerPostings = postings.filter(posting => posting.postingType == answerType).map(posting => (posting.parentId.get, posting))

    val qaRDD = questionPostings.join(answerPostings)
    qaRDD.groupByKey()


//    val addToList = (list: List[(Int, (Posting, Posting))], v: (Posting) => v :: list
//    val sumPartitionLists = (list1: List[(Posting, Posting)], list2: List[(Posting, Posting)]) => list1 ::: list2

//    qaRDD.aggregateByKey(List)(addToList , sumPartitionLists)
  }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map { case (_, iter) =>
      ...
      val answers = iter.map{ case (_, b) => b}.toArray
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    val vectors = scored.map{ case(question, highScore) =>
        val langIndex = firstLangInTag(question.tags, langs)
        (langIndex, highScore)
      }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val cluster: Array[(Int, (Int, Int))] = vectors
