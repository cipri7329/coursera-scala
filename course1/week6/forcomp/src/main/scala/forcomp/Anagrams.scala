...
object Anagrams {
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.flatMap( word => Map(wordOccurrences(word) -> word))
    ...
    def sentenceAnagramsIter(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          ...
          word <- dictionaryByOccurrences getOrElse (occurencesCombination, List())
          rest <- sentenceAnagramsIter(subtract(occurrences, occurencesCombination))
        } yield word :: rest
      }
    }
