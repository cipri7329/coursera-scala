...
case class LocationDateTemp(id: String, latitude:Double, longitude: Double, day: Int, month: Int, year: Int, temperature: Double)

case class StationDate(day: Int, month: Int, year: Int){
  def toLocalDate = LocalDate.of(year, month, day)
}

case class StationDateLocationTemp(date: StationDate, location: Location, temperature: Double)


/**
  * STN identifier	WBAN identifier
  * Latitude	Longitude
  * @param id
  * @param latitude
  * @param longitude
