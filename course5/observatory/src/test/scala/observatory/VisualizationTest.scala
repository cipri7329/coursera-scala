...
trait VisualizationTest extends FunSuite with Checkers with SparkJob {

  lazy val locateTemperatures2 = Extraction.locateTemperatures(year2, stationsPath2, temperaturePath2)
  lazy val locateAverage2 = Extraction.locationYearlyAverageRecords(locateTemperatures2)
  val year2 = 1975
  val debug2 = true
  val stationsPath2: String = "/observatory/stations.csv"
  val temperaturePath2: String = s"/observatory/$year2-sample50k.csvg"


  test("locationYearlyAverageRecords2") {
    if (debug2) locateAverage2.take(20).foreach(println)
    assert(locateAverage2.count(_._1 == Location(70.933, -8.667)) === 1)
    assert(locateAverage2.size === 215)
  }
  ...
  test("Distance 0.0") {
    assert(Visualization.predictTemperature(locateAverage2, Location(67.55, -63.783)).round === 4)
    assert(Visualization.predictTemperature(locateAverage2, Location(39.083, -76.767)).round === 13)
  }

  test("Distance != 0.0") {
    assert(Visualization.predictTemperature(locateAverage2, Location(52.0, 4.5)).round === 6)
    assert(Visualization.predictTemperature(locateAverage2, Location(4.5, 52.0)).round === 13)
    assert(Visualization.predictTemperature(locateAverage2, Location(0.0, 0.0)).round === 7)
  ...
  test("visualize") {
    ...
    val img = Visualization.visualize(locateAverage2, palette)

    img.output(new java.io.File(s"../../src/test/resources/$year2-sample50k.png"))
