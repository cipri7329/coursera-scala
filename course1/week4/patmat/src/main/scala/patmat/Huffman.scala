...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Fork(left, right, chars, treeWeight) => treeWeight
...
//    traverse(tree, bits)
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = {
      table match {
        case (ch, bitList) :: Nil if ch.equals(char) => bitList
        case (ch, bitList) :: _ if ch.equals(char) => bitList
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      ...
      def convert0(treeP: CodeTree) : CodeTable = treeP match {
//        case lf : Leaf => List((lf.char, encode(lf)(List(lf.char))))
//        case Fork(left, right, _, _) => mergeCodeTables(convert0(left), convert0(right))
